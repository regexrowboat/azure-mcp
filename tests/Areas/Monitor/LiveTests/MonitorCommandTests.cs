...
public class MonitorCommandTests(LiveTestFixture fixture, ITestOutputHelper output) : CommandTestsBase(fixture, output), IClassFixture<LiveTestFixture>, IAsyncLifetime
{
    ...
    [Fact]
    [Trait("Category", "Live")]
    public async Task Should_list_metric_namespaces()
    {
        // Example resource ID - uses a storage account that should exist from the test fixture
        string resourceId = $"/subscriptions/{Settings.SubscriptionId}/resourceGroups/{Settings.ResourceGroupName}/providers/Microsoft.Storage/storageAccounts/{Settings.ResourceBaseName}";

        var result = await CallToolAsync(
            "azmcp-monitor-metrics-namespaces",
            new()
            {
                { "subscription", Settings.SubscriptionId },
                { "resource-name", Settings.ResourceBaseName },
        ...
        // Validate at least one namespace has all expected properties populated
        var firstNamespace = resultsArray.EnumerateArray().First();

        // Verify required properties exist and are populated
        Assert.True(firstNamespace.TryGetProperty("name", out var name));
        Assert.Equal(JsonValueKind.String, name.ValueKind);
        Assert.False(string.IsNullOrEmpty(name.GetString()));

        Assert.True(firstNamespace.TryGetProperty("type", out var type));
        Assert.Equal(JsonValueKind.String, type.ValueKind);
        Assert.False(string.IsNullOrEmpty(type.GetString()));

        Assert.True(firstNamespace.TryGetProperty("classificationType", out var classificationType));
        Assert.Equal(JsonValueKind.String, classificationType.ValueKind);
        Assert.False(string.IsNullOrEmpty(classificationType.GetString()));
    }
}
