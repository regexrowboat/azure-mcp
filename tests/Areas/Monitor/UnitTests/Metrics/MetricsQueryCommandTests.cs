...
public class MetricsQueryCommandTests
{
    ...
    {
        // Arrange
        var args = "--subscription sub1 --resource-name sa1 --metric-names CPU";
        ...
        await _service.Received(1).QueryMetricsAsync(
            ...
            Arg.Is<string?>(t => t == null), // metric namespace (not provided)
    ...
    {
        // Arrange
        var args = $"--subscription sub1 --resource-name sa1 --metric-names \"{metricNames}\"";
        var parseResult = _command.GetCommand().Parse(args);
    ...
    [Theory]
    [InlineData("--subscription sub1 --resource-name sa1 --metric-names CPU")]
    [InlineData("--subscription sub1 --resource-group rg1 --resource-type Microsoft.Storage/storageAccounts --resource-name sa1 --metric-names CPU")]
    [InlineData("--subscription sub1 --resource-name sa1 --metric-names CPU,Memory")]
    [InlineData("--subscription sub1 --resource-name sa1 --metric-names CPU --start-time 2023-01-01T00:00:00Z --end-time 2023-01-02T00:00:00Z")]
    ...
    public async Task ExecuteAsync_EmptyResults_ReturnsSuccessWithNullResults()
    {
        ...
        var context = new CommandContext(_serviceProvider);
        var parseResult = _command.GetCommand().Parse("--subscription sub1 --resource-name sa1 --metric-names CPU");
    ...
    public async Task ExecuteAsync_CallsServiceWithCorrectParameters()
    {
        ...
        var parseResult = _command.GetCommand().Parse(
            "--subscription sub1 --resource-group rg1 --resource-type Microsoft.Storage/storageAccounts " +
    ...
    public async Task ExecuteAsync_ExceedsCustomBucketLimit_ReturnsBadRequest()
    {
        ...
        var context = new CommandContext(_serviceProvider);
        var parseResult = _command.GetCommand().Parse("--subscription sub1 --resource-name sa1 --metric-names Memory --max-buckets 25");
    ...
    public async Task ExecuteAsync_ChecksAllBucketTypes_ForLimitExceeded(string bucketType)
    {
        ...
        var context = new CommandContext(_serviceProvider);
        var parseResult = _command.GetCommand().Parse("--subscription sub1 --resource-name sa1 --metric-names TestMetric");
    ...
    public async Task ExecuteAsync_MultipleMetricsWithMixedBucketCounts_ValidatesEach()
    {
        ...
        var context = new CommandContext(_serviceProvider);
        var parseResult = _command.GetCommand().Parse("--subscription sub1 --resource-name sa1 --metric-names CPU,Memory");
