    ...
    : BaseAzureService(), IMonitorMetricsService
{
        ...
        RetryPolicyOptions? retryPolicy = null)
    {
        ...
        try
        {
            ...
            {
                if (TimeSpanHelpers.TryParseISO8601Duration(interval, out var intervalTimeSpan))
                {
                    queryOptions.Granularity = intervalTimeSpan;
                }
                else
                {
                    throw new ArgumentException($"Invalid interval format: {interval}. Expected ISO 8601 duration format (e.g., PT1M, PT5M, PT1H, P1D)");
                }
            }
        ...
        {
            throw new Exception($"Error querying metrics: {ex.Message}", ex);
        }
    }
        ...
        RetryPolicyOptions? retryPolicy = null)
    {
                ...
                foreach (global::Azure.Monitor.Query.Models.MetricDefinition definition in page.Values)
                {
                    ...
                    if (definition.MetricAvailabilities != null)
                    {
                        metricDef.AllowedIntervals = definition.MetricAvailabilities
                            ...
                            .Select(a => TimeSpanHelpers.FormatTimeSpanAsISO8601(a.Granularity!.Value))
                            .Distinct()
        ...
        {
            throw new Exception($"Error listing metric definitions: {ex.Message}", ex);
        }
    }
        ...
        RetryPolicyOptions? retryPolicy = null)
    {
        ...
        try
        {
            ...
            await foreach (var page in pages)
            {
                foreach (global::Azure.Monitor.Query.Models.MetricNamespace ns in page.Values)
                {
                    ...
                    {
                        Name = (ns.Name ?? string.Empty).Replace("-", "/"),
        ...
        {
            throw new Exception($"Error listing metric namespaces: {ex.Message}", ex);
        }
    }
